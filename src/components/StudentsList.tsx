...
import { ListIcon, RoomIcon, DoorIcon, TelegramIcon, CheckIcon, CloseIcon, EditIcon, EyeIcon } from '@/components/Icons';
...
export default function StudentsList() {
  ...
  if (!students || students.length === 0) {
    return (
      <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
        ...
        <p className="text-gray-700 text-lg">Студентов пока нет.</p>
      </div>
    );
  }
  ...
  const renderMobileRow = (student: Student, index: number, list: Student[], tone: 'blue' | 'green') => {
    ...
    return (
      <React.Fragment key={student.id}>
        {showDivider && (
          <tr className={tone === 'blue' ? 'bg-gradient-to-r from-transparent via-blue-300 to-transparent' : 'bg-gradient-to-r from-transparent via-green-300 to-transparent'}>
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-lg border border-gray-200">
        ...
        {notice && (
          <div className={`mx-4 mb-4 rounded-lg border px-3 py-2 text-sm ${notice.type === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-red-200 bg-red-50 text-red-700'}`}>
            {notice.message}
          </div>
        )}
              ...
              {isSuperAdmin && (
                <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-lg">
                  ...
                  <label htmlFor="canViewStudents" className="text-sm font-semibold text-gray-900 cursor-pointer flex items-center gap-1">
                    <EyeIcon className="w-4 h-4" />Доступ к вкладке студентов
                  </label>
                </div>
              )}
            </div>
