...
\restrict Ha2h0NNnKf7duzmcgARzzWhU9JWVOU2G9lFVI8Xos1vv0Gng7cWbIGtoe9T1pPK
...
CREATE FUNCTION public.get_queue_active() RETURNS TABLE(id uuid, student_id uuid, full_name text, room text, wash_count integer, payment_type text, joined_at timestamp with time zone, planned_start_at timestamp with time zone, expected_finish_at timestamp with time zone, finished_at timestamp with time zone, note text, admin_message text, return_key_alert boolean, admin_room text, ready_at timestamp with time zone, key_issued_at timestamp with time zone, washing_started_at timestamp with time zone, washing_finished_at timestamp with time zone, return_requested_at timestamp with time zone, status text, scheduled_for_date date, queue_date date, queue_position integer, avatar_type text)
    LANGUAGE sql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
  select
...
\unrestrict Ha2h0NNnKf7duzmcgARzzWhU9JWVOU2G9lFVI8Xos1vv0Gng7cWbIGtoe9T1pPK
